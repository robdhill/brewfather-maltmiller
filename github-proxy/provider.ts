...
// register itself and complete a "Sign in with GitHub" login against
// defaultHandler (see oauth-handler.ts), which verifies the signed-in
// account matches ALLOWED_GITHUB_USERNAME before granting anything. Once
// authorized, every /gh-proxy/mcp request is handled by apiHandler above,
// which proxies onward to GitHub's real remote MCP server using a separate,
// fine-grained PAT (GITHUB_PROXY_PAT) — two independent credentials serving
